"""IPC command dispatcher for the sidecar.

Routes a single parsed JSON-RPC command dict to the appropriate handler.
Extracted from the monolithic ``src/main.py``. Shared stateful globals
(``_db_conn``, ``_start_time``, ``POLL_INTERVAL``) and a few helper symbols
remain owned by ``src.main`` (the application root / entry point) so the
existing ``tests/test_main.py`` patches (``patch("src.main.X")``) keep
working. This module reads those symbols lazily from ``src.main`` at call
time rather than binding them at import time, which avoids a circular import
and keeps the patched attributes live.
"""
...
def _handle_command(
    ...
    # Read the live persistent connection from the application root so a
    # ``patch("src.main._db_conn")`` in tests is honoured.
    conn = main_mod._db_conn
    ...
    try:
        ...
        if method == "system.health":
            ...
            if main_mod._db_conn is not None:
                leads_count = main_mod._db_conn.execute(
            ...
            return _success_response(
                ...
                {
                    "status": "ok",
                    "uptime_seconds": int(time.time() - main_mod._start_time),
                    "version": "0.1.0",
        ...
        if method == "schema.apply":
            main_mod.apply_schema(conn)
            main_mod.run_migrations(conn)
        ...
        if method == "entity.unmask":
            ...
            try:
                llm = main_mod.get_active_llm_client(conn)
            except ValueError:
                llm = None
            apn = params.get("apn", "")
        ...
        if method == "captcha.detect":
            handler = CaptchaHandler(main_mod._db_conn)
        ...
        if method == "llm_settings.set":
            ...
            if api_key and not api_key.endswith("****"):
                main_mod.store_credential(f"llm/{provider}/api_key", api_key)
            conn.execute(
        ...
        if method == "llm_settings.get":
            ...
            for r in rows:
                ...
                if d.get("api_key"):
                    ...
                    real_key = main_mod.get_credential(keychain_key)
                    if real_key:
